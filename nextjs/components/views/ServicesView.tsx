      ...
      >
        <nav aria-label="Services on this page" data-reveal data-delay="0.45" className="mt-10 flex flex-wrap justify-center gap-2">
          {services.map((service) => (
            <Link key={service.slug} href={serviceHref(service.slug)} className="rounded-full border border-line px-4 py-2.5 text-[0.9rem] transition-colors hover:border-ink">
              {service.title}
            </Link>
          ))}
        </nav>
