  ...
  return (
    ...
    >
      <div className="container-x flex h-20 items-center justify-between lg:grid lg:grid-cols-[1fr_auto_1fr]">
        <nav aria-label="Main" className="hidden items-center gap-8 lg:flex">
          {navLinks.map((link) => (
            <NavLink
              key={link.to}
              to={link.to}
              className={({ isActive }) => `relative py-2 text-[0.95rem] after:absolute after:inset-x-0 after:bottom-1 after:h-px after:origin-left after:bg-current after:transition-transform after:duration-500 hover:after:scale-x-100 ${isActive ? 'after:scale-x-100' : 'after:scale-x-0'}`}
            >
              {link.label}
            </NavLink>
          ))}
        ...
        <Link to="/" aria-label="Tarun Makhija — home" className="font-serif text-[1.65rem] leading-none font-semibold tracking-tight lg:text-[1.85rem]">
          Tarun Makhija
        </Link>
